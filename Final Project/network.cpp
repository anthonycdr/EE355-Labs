...
void Network::loadDB(string filename){
    ...
    while (getline(file, fname)) {
        if (fname.empty()) continue;                  // skip blank lines
        string lname, bdate, emailLn, phoneLn,
                    collegeLn, majorLn, zipLn, divider;
        vector <pair<string,string>> temp;
        getline(file, lname);
        getline(file, bdate);
        getline(file, emailLn);                       
        getline(file, phoneLn);
        getline(file, collegeLn);
        getline(file, majorLn);
        getline(file, zipLn);
        ...
        Person* np = new Person(fname, lname, bdate,
            emailLn, phoneLn,
            collegeLn, majorLn, zipLn);
        People_friend_list.push_back(temp);
        push_back(np); // Add person to the network
    }
    file.close();
    Person* traverse = head;
...
void Network::saveDB(string filename) {
    ...
    while (ptr != NULL) {
        ...
        file << "(College): " << ptr->get_school() << '\n';
        file << "(Major): "   << ptr->get_major()  << '\n';
        file << "Zip code: " << ptr->get_zip() << '\n';
...
static std::string lowerCopy(string s)
{
    for (char &c : s) c = tolower(static_cast<unsigned char>(c));
    return s;
}

void Network::wiseSearch()
{
    if (!head) { cout << "Database is empty.\n"; return; }     // if there's no remaining 

    vector<Person*> pool;
    for (Person* p = head; p; p = p->next) pool.push_back(p);   // get every person in the network

    while (true)
    {
        cout << "\nRemaining matches: " << pool.size() << '\n';

        if (pool.empty()) { cout << "No records fit.\n"; return; }
        if (pool.size() == 1) {
            cout << "Unique Person Found:\n";
            pool.front()->print_person();
            return;
        }
        else if (pool.size() <= 8){
        	for (Person* p : pool) {
        ...
        cout << "Filter by field\n"
                     " first | last | college | major | zip | area | email | phone | dob\n"
                     "Field (or q to quit): ";

        string field;
        getline(cin >> ws, field);

        field = lowerCopy(field);
        if (field == "q" || field == "quit") return;

        cout << "Value (substring, case‑insensitive): ";
        string value;
        getline(cin >> ws, value);
        string vLower = lowerCopy(value);

        vector<Person*> next;
        for (Person* p : pool) {
            string candidate = lowerCopy(p->getField(field));
            if (!candidate.empty() &&
                candidate.find(vLower) != string::npos)
                next.push_back(p);
        }
        pool.swap(next);
    }
